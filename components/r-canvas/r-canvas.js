 ...
 * https://gitee.com/citn/r-canvas-example
 */
export default {
  data() {
    return {
  ...
  methods: {
      ...
      return new Promise(async (resolve, reject) => {
        if (config.text) {
          ...
          if (configuration.text_width >= this.ctx.measureText(config.text).width) {
            ...
            draw_width =
              max_width < this.ctx.measureText(config.text).width
                ? this.resetCompatibilitySize(max_width)
                : this.resetCompatibilitySize(this.ctx.measureText(config.text).width)
          } else {
              ...
              if (this.compatibilitySize(this.ctx.measureText(current_text + text_arr[i]).width) > this.compatibilitySize(max_width)) {
                ...
                if (config.line_clamp && parseInt(config.line_clamp) == 1) {
                  ...
                  while (true) {
                    ...
                    if (
                      this.compatibilitySize(this.ctx.measureText(json_current_text).width) <=
                      this.compatibilitySize(this.ctx.measureText(line_clamp_hint).width)
                    ) {
                  ...
                  this.ctx.fillText(
                    // current_text + line_clamp_hint,
                    current_text.substr(0, current_text.length - 1) + line_clamp_hint, // 卢忠伟
                    this.compatibilitySize(parseFloat(config.x) * this.scale),
                    this.compatibilitySize(parseFloat(config.y) * this.scale)
                  )
                } else {
                  configuration.line_num += 1
                  this.ctx.setFontSize(parseInt(this.compatibilitySize(font_size))) // font size
    ...
    drawRect(config) {
      return new Promise(async (resolve, reject) => {
        ...
        if (config.is_radius || config.border_radius) {
          ...
          this.ctx.fillRect(
            this.compatibilitySize(config.x * this.scale),
            this.compatibilitySize(config.y * this.scale),
            this.compatibilitySize(parseFloat(config.w) * this.scale),
            this.compatibilitySize(parseFloat(config.h) * this.scale)
          )
        }
        resolve()
      })
      ...
      return new Promise(async (resolve, reject) => {
        if (config.url) {
          ...
          if (reg.test(config.url)) {
            ...
            if (
              config.url.indexOf('data:image/png;base64') != -1 ||
              config.url.indexOf('data:image/jpeg;base64') != -1 ||
              config.url.indexOf('data:image/gif;base64') != -1
            ) {
              type = 3
            } else {
              type = 2
          ...
          if (type == 1) {
            // network image
            await this.downLoadNetworkFile(config.url)
              .then(res => {
                // two function
                image_url = res
              })
            ...
            // #ifdef MP-WEIXIN
            await this.base64ToNative({ url: base64 })
              .then(res => {
                image_url = res
              })
              .catch(err => {
            ...
            // #ifdef MP-WEIXIN
            await this.base64ToNative({ url: config.url })
              .then(res => {
                image_url = res
              })
              .catch(err => {
          ...
          if (config.border_radius) {
            ...
            this.ctx.beginPath()
            this.ctx.arc(
              this.compatibilitySize(parseFloat(config.x) * this.scale + (parseFloat(config.w) * this.scale) / 2),
              this.compatibilitySize(parseFloat(config.y) * this.scale + (parseFloat(config.h) * this.scale) / 2),
              this.compatibilitySize((parseFloat(config.w) * this.scale) / 2),
              0,
              2 * Math.PI,
              false
            )
    ...
    setNativeBorderRadius(config) {
      ...
      this.ctx.arcTo(
        this.compatibilitySize(parseFloat(config.x) * this.scale + parseFloat(config.w) * this.scale),
        this.compatibilitySize(parseFloat(config.y) * this.scale),
        this.compatibilitySize(parseFloat(config.x) * this.scale + parseFloat(config.w) * this.scale),
        this.compatibilitySize(parseFloat(config.y) * this.scale + parseFloat(config.h) * this.scale),
        this.compatibilitySize(border_radius)
      )
      this.ctx.arcTo(
        this.compatibilitySize(parseFloat(config.x) * this.scale + parseFloat(config.w) * this.scale),
        this.compatibilitySize(parseFloat(config.y) * this.scale + parseFloat(config.h) * this.scale),
