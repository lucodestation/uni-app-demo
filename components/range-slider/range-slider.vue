...
export default {
  ...
  props: {
    ...
    //live模式，是否动态更新
    // 所谓动态更新是滑动滑块过程中触发 rangechange
    // 设为 false 则是在滑动结束后触发 rangechange
    liveMode: {
      type: Boolean,
      default: true,
    },
  ...
  watch: {
    ...
    values: function (newVal, oldVal, changedPath) {
      ...
      // _isValuesValid 判断是否是有效的值
      // 不为 null 不为 undefined 长度为 2 的数组即为有效的值
  ...
  methods: {
    ...
    formatNumber(num, step) {
      ...
      return (
        (parseInt(tmpNum / step + (step > 1 ? 1 : step) * 0.5) * step) /
        offestNum
      ).toFixed(len)
    },
  },
...
宽 16rpx
 */
.bubble-text .triangle {
  position: absolute;
  bottom: -10rpx;
  border-top: 12rpx solid #2a2a2a;
